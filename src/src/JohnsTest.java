...
public static final Map m = new Map(20, 20);
/**
...
Avatar a = new Avatar("a", '~');
Terrain obstacle = new Terrain("boulder", 'O', true, false);
MapView map_view = new MapView();
obstacle.addDecal('✚');
m.initializeTerrain(obstacle, 1, 2);
System.out.println("Adding avatar. Error code: " + m.addAvatar(a, 0, 0));

Item equipable = new Item("i", 'i', true);
m.addItem(equipable, 0, 0, true, true);
int error_code1 = a.equipInventoryItem();

System.out.println("top: " + m.getTile(0, 0).getTopCharacter() + error_code1);
a.getMapRelation().pickUpItemInDirection(0, 0);
int error_code2 = a.equipInventoryItem();
System.out.println("top: " + m.getTile(0, 0).getTopCharacter()+ error_code2);

System.out.println(m.getTile(2, 0).isPassable());
System.out.println(m.getTile(0, 2).isPassable());
System.out.println(m.getTile(0, 0).isPassable());
...
_d.printView();
map_display.associateWithMap(m);
...
System.out.println(
m.getTile(a.getMapRelation().getMyXCoordinate(),
